...
track_pcs_df <- track_features %.% {
  ...
  left_join(track_to_playlist_map, by = c("track_name", "track_id"))
...
```{r, fig.height=8}
plot_track_pca <- function(df, color, title, size = NULL, alpha = 0.6) {
  df %>%
...
plot_track_pca(track_pcs_df, playlist, "PCA: Playlist") +
...
track_pcs_df %>%
  ...
  select(track_name, track_id, PC1, PC2, playlist_grp) %>%
  group_by(track_name, track_id, PC1, PC2, playlist_grp) %>%
  distinct() %>%
  summarise(playlist_grp = ifelse(n() > 1, "multiple", playlist_grp)) %>%
  ungroup() %>%
  plot_track_pca(playlist_grp, "PCA: Playlist Category") +
  scale_color_brewer(
